...
{
    public static Enumeratorable<T, γWhere<T, γMemory<T>>> Where<T, TEnumeratorable>(this ImmutableArray<T> source, Func<T, bool> predicate)
        where TEnumeratorable : struct, IEnumeratorable<T> =>
        new(new(new(source), predicate));

    public static Enumeratorable<U, γSelect<T, U, γMemory<T>>> Select<T, U, TEnumeratorable>(this ImmutableArray<T> source, Func<T, U> selector)
        where TEnumeratorable : struct, IEnumeratorable<T> =>
